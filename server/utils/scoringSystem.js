// utils/scoringSystem.js (Server-side version)
// DLI-R Validated Scoring System - EXACT copy from frontend
...
const calculateDLIDomainScore = (answers, domain) => {
...
const calculateDLIScores = (answers) => {
  const domainScores = {
...
const compareWithDLIBenchmarks = (userScores) => {
  const comparisons = {};
...
const generateDLIFeedback = (comparisons, demographics) => {
  const feedback = {
    summary: '',
    strengths: [],
    improvements: [],
...
// Social Connection calculation (from your ResultsPage)
const calculateSocialConnection = (answers) => {
...
const validateDLICompleteness = (answers) => {
  const completion = {};
...
module.exports = {
  ...
  validateDLICompleteness,
  getDLIDomainMapping: () => DLI_QUESTION_MAPPING,
  getNationalBenchmarks: () => NATIONAL_BENCHMARKS
};